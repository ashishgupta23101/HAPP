...
export class HelperService {
  ...
  GetCarrierCode(TrackingNo) {
    ...
    const trackingLength = TrackingNo.length;
    // logic for Ontrac
    ...
    switch (trackingLength)
    {
      ...
      case 12: // FedEx Express,FedEx Freight,Purolator
            const check = this.CheckFXXCheckDigit(TrackingNo);
            if (check === true) {
          CarrierCode = 'F';
...
USPS_CheckDigit(Tracking_No){
  let tracking_no = '';
  let checkDigit = Tracking_No.charAt(Tracking_No.length - 1);
  let sum = 0;
  let odd = 0;
  let even = 0;
  let getDigit = 0;
